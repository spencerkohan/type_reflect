...
// Shared with the `untagged` (bare-content) module: emits the per-case
// content validator namespace (`{Parent}Case{Case}`).
pub use case_type::emit_case_type;

/// Serde's default (external) representation: unit cases are the case-name
/// string, tuple/struct cases are `{ "CASE": content }`.
pub fn emit_externally_tagged_enum_type<T>() -> String
where
